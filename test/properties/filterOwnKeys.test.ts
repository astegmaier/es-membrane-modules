...
describe("Filtering own keys ", function () {
  ...
  function definePropertyTests(modifyFilter: () => void) {
    ...
    it("hides defined properties from getters", function () {
    ...
    it("does not affect setting or deleting a (configurable) property that isn't blacklisted", function () {
      var keys;

      // Set extra initially to 3.
      const isExtensible = Reflect.isExtensible(wetDocument);
      expect(Reflect.defineProperty(dryDocument, "extra", extraDesc)).toBe(isExtensible);
      ...
      expect(Reflect.defineProperty(dryDocument, "extra", extraDesc2)).toBe(isExtensible);
    ...
    describe(".defineProperty(dryDocument, 'blacklisted', desc) returns false for a blacklisted property, and does not set the property", function () {
      ...
      it("where desc.configurable is false,", function () {
        ...
        expect(Reflect.defineProperty(dryDocument, "blacklisted", desc)).toBe(false);
    ...
    describe(".deleteProperty(dryDocument, 'blacklisted') returns true for a blacklisted property", function () {
      it("when the property was never defined", function () {
        expect(Reflect.deleteProperty(dryDocument, "blacklisted")).toBe(true);
        checkDeleted();
      });

      it("when the property was previously defined on the wet graph as configurable", function () {
        const isExtensible = Reflect.isExtensible(wetDocument);
        // Set extra initially to 3.
      ...
      it("when the property was previously defined on the wet graph as non-configurable", function () {
        const isExtensible = Reflect.isExtensible(wetDocument);
        // Set extra initially to 3.
      ...
      it("when the property's definition on the dry graph was attempted", function () {
    ...
    describe(".deleteProperty(dryDocument, 'blacklisted') triggers a membrane logger warning once", function () {
      beforeEach(function () {
        rebuildMocksWithLogger();
      });
      ...
      it("when the property was previously defined on the wet graph as configurable", function () {
        // Set extra initially to 3.
        const wasDefined = Reflect.defineProperty(wetDocument, "blacklisted", extraDesc);
        appender.clear();
        expect(Reflect.deleteProperty(dryDocument, "blacklisted")).toBe(true);
        checkDeleted();
  ...
  [voidFunc, sealWetDocument, sealDryDocument, freezeWetDocument, freezeDryDocument].forEach(
    defineTestsBySealant
  );
  ...
  describe("with the damp object graph (not affecting dry or wet)", function () {
    ...
    it("does not hide defined properties from getters", function () {
      let keys = Reflect.ownKeys(dryDocument);
      fixKeys(keys);
      expect(keys.includes("nodeType")).toBe(true);
      expect(keys.includes("__events__")).toBe(true);
    ...
    it("does not affect setting or deleting a (configurable) property that isn't blacklisted", function () {
      ...
      // Set extra initially to 3.
      expect(Reflect.defineProperty(dryDocument, "extra", extraDesc)).toBe(true);
      ...
      // Set extra again, to 4.
      expect(Reflect.defineProperty(dryDocument, "extra", extraDesc2)).toBe(true);
    ...
    describe(".defineProperty(dryDocument, 'blacklisted', desc) returns true for a blacklisted property, and sets the property", function () {
      ...
      it("where desc.configurable is false,", function () {
        desc.configurable = false;
        expect(Reflect.defineProperty(dryDocument, "blacklisted", desc)).toBe(true);
    ...
    describe(".deleteProperty(dryDocument, 'blacklisted') returns true for a blacklisted property", function () {
      ...
      it("when the property was previously defined on the wet graph as non-configurable", function () {
        ...
        expect(Reflect.deleteProperty(dryDocument, "blacklisted")).toBe(false);

        // Test that the delete didn't apply to the dry object graph.
        {
          let desc = Reflect.getOwnPropertyDescriptor(dryDocument, "blacklisted");
    ...
    describe(".deleteProperty(dryDocument, 'blacklisted') does not trigger a membrane logger warning", function () {
      beforeEach(function () {
        rebuildMocksWithLogger();
      });
