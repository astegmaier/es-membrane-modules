...
describe("TC39 demonstrations of Array objects in membranes: ", function () {
  ...
  function populateDry() {
    ...
    expect(parts.dry.array).toEqual([parts.dry.alpha, parts.dry.beta, parts.dry.gamma]);
  }
  ...
  it("Without distortions, mirroring functions normally", function () {
    ...
    expect(parts.wet.array).toEqual([parts.wet.omega, parts.wet.epsilon, parts.wet.gamma]);

    expect(parts.dry.array).toEqual([parts.dry.omega, parts.dry.epsilon, parts.dry.gamma]);
  });

  it("A natural call to .splice operates on the native graph, but doesn't respect storeUnknownAsLocal", function () {
